...
dw.IDIMPRESSION_LOG,
fp.IDIMPRESSION_LOG as processed_idimpression_log,
case when dw.IDIMPRESSION_LOG = fp.IDIMPRESSION_LOG then 'Correct - IDIMPRESSION_LOG' else 'Incorrect - IDIMPRESSION_LOG' end as idimpression_log_flag,
dw.EVENT_DATE,
...
dw.RULE_FLAGS,
fp.RULE_FLAGS as processed_rule_flags,
case 
...
dw.GEO_DB,
fp.GEO_DB as processed_geo_db,
case 
    when dw.GEO_DB is null and fp.GEO_DB is null then 'Correct - GEO_DB' 
    when dw.GEO_DB = fp.GEO_DB then 'Correct - GEO_DB' 
    else 'Incorrect - GEO_DB' 
end as geo_db_flag,
dw.FT_CUSTOM,
...
dw.PLACEMENT_TREE_ID,
fp.PLACEMENT_TREE_ID as processed_placement_tree_id,
case 
    when dw.PLACEMENT_TREE_ID is null and fp.PLACEMENT_TREE_ID is null then 'Correct - PLACEMENT_TREE_ID' 
    when dw.PLACEMENT_TREE_ID = fp.PLACEMENT_TREE_ID then 'Correct - PLACEMENT_TREE_ID' 
    else 'Incorrect - PLACEMENT_TREE_ID' 
end as placement_tree_id_flag,
dw.PLACEMENT_TREE_TYPE,
fp.PLACEMENT_TREE_TYPE as processed_placement_tree_type,
case 
    when dw.PLACEMENT_TREE_TYPE is null and fp.PLACEMENT_TREE_TYPE is null then 'Correct - PLACEMENT_TREE_TYPE' 
    when dw.PLACEMENT_TREE_TYPE = fp.PLACEMENT_TREE_TYPE then 'Correct - PLACEMENT_TREE_TYPE' 
    else 'Incorrect - PLACEMENT_TREE_TYPE' 
end as placement_tree_type_flag,
dw.PLACEMENT_PATH,
fp.PLACEMENT_PATH as processed_placement_path,
case 
    when dw.PLACEMENT_PATH is null and fp.PLACEMENT_PATH is null then 'Correct - PLACEMENT_PATH' 
    when dw.PLACEMENT_PATH = fp.PLACEMENT_PATH then 'Correct - PLACEMENT_PATH' 
    else 'Incorrect - PLACEMENT_PATH' 
end as placement_path_flag,
dw.CREATIVE_TREE_ID,
fp.CREATIVE_TREE_ID as processed_creative_tree_id,
case 
    when dw.CREATIVE_TREE_ID is null and fp.CREATIVE_TREE_ID is null then 'Correct - CREATIVE_TREE_ID' 
    when dw.CREATIVE_TREE_ID = fp.CREATIVE_TREE_ID then 'Correct - CREATIVE_TREE_ID' 
    else 'Incorrect - CREATIVE_TREE_ID' 
end as creative_tree_id_flag,
dw.CREATIVE_TREE_TYPE,
fp.CREATIVE_TREE_TYPE as processed_creative_tree_type,
case 
    when dw.CREATIVE_TREE_TYPE is null and fp.CREATIVE_TREE_TYPE is null then 'Correct - CREATIVE_TREE_TYPE' 
    when dw.CREATIVE_TREE_TYPE = fp.CREATIVE_TREE_TYPE then 'Correct - CREATIVE_TREE_TYPE' 
    else 'Incorrect - CREATIVE_TREE_TYPE' 
end as creative_tree_type_flag,
dw.CREATIVE_PATH,
fp.CREATIVE_PATH as processed_creative_path,
case 
    when dw.CREATIVE_PATH is null and fp.CREATIVE_PATH is null then 'Correct - CREATIVE_PATH' 
    when dw.CREATIVE_PATH = fp.CREATIVE_PATH then 'Correct - CREATIVE_PATH' 
    else 'Incorrect - CREATIVE_PATH' 
end as creative_path_flag,
dw.IDFT_AGENTS_OS,
...
dw.IDVIEWABLEEVENTTYPE,
fp.IDVIEWABLEEVENTTYPE as processed_idviewableeventtype,
case 
    when dw.IDVIEWABLEEVENTTYPE is null and fp.IDVIEWABLEEVENTTYPE is null then 'Correct - IDVIEWABLEEVENTTYPE' 
    when dw.IDVIEWABLEEVENTTYPE = fp.IDVIEWABLEEVENTTYPE then 'Correct - IDVIEWABLEEVENTTYPE' 
    else 'Incorrect - IDVIEWABLEEVENTTYPE' 
end as idviewableeventtype_flag,
dw.DEVICE_ID,
...
dw.DOMAIN,
fp.DOMAIN as processed_domain,
case 
    when dw.DOMAIN is null and fp.DOMAIN is null then 'Correct - DOMAIN' 
    when dw.DOMAIN = fp.DOMAIN then 'Correct - DOMAIN' 
    else 'Incorrect - DOMAIN' 
end as domain_flag,
dw.FT_PRODUCT,
...
dw.FULL_COUNTRY_CODE,
fp.FULL_COUNTRY_CODE as processed_full_country_code,
case 
    when dw.FULL_COUNTRY_CODE is null and fp.FULL_COUNTRY_CODE is null then 'Correct - FULL_COUNTRY_CODE'
    when dw.FULL_COUNTRY_CODE = fp.FULL_COUNTRY_CODE then 'Correct - FULL_COUNTRY_CODE' 
    else 'Incorrect - FULL_COUNTRY_CODE' 
end as full_country_code_flag,
dw.BROWSER_VERSION,
fp.BROWSER_VERSION as processed_browser_version,
case 
    when dw.BROWSER_VERSION is null and fp.BROWSER_VERSION is null then 'Correct - BROWSER_VERSION' 
    when dw.BROWSER_VERSION = fp.BROWSER_VERSION then 'Correct - BROWSER_VERSION' 
    else 'Incorrect - BROWSER_VERSION' 
end as browser_version_flag,
...
case 
    when dw.FT_C3 is null and fp.FT_C3 is nul then 'Correct - FT_C3' 
...
dw.RUN_DATEHOUR
From
    "FT_DB_DEV"."DATA_WAREHOUSE"."IMP" as dw
Full outer join
    "FT_DB_DEV"."FT_PROCESSED"."IMP" as fp
    On  dw.IDLOG_FILE_CONTROL = fp.IDLOG_FILE_CONTROL
    And dw.IDIMPRESSION_LOG = fp.IDIMPRESSION_LOG
...
Or idimpression_log_flag like 'Incorrect%'
...
Or geo_db_flag like 'Incorrect%'
Or ft_custom_flag like 'Incorrect%'
Or ft_section_flag like 'Incorrect%'
Or privacy_flag_flag like 'Incorrect%'
Or placement_tree_id_flag like 'Incorrect%'
Or placement_tree_type_flag like 'Incorrect%'
Or placement_path_flag like 'Incorrect%'
Or creative_tree_id_flag like 'Incorrect%'
Or creative_tree_type_flag like 'Incorrect%'
Or creative_path_flag like 'Incorrect%'
...
Or impression_id_flag like 'Incorrect%'
Or idviewableeventtype_flag like 'Incorrect%'
Or device_id_flag like 'Incorrect%'
Or user_ids_flag like 'Incorrect%'
Or idagent_env_flag like 'Incorrect%'
Or domain_flag like 'Incorrect%'
Or ft_product_flag like 'Incorrect%'
Or ft_data_flag like 'Incorrect%'
Or full_country_code_flag like 'Incorrect%'
Or browser_version_flag like 'Incorrect%'
